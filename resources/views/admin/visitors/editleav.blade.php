...
<div class="card">
    ...
    <div class="card-body">
        <form name="myForm" method="POST" action="{{ route("admin.visitorsleav.update", [$visitorsleav->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
<!--            <input type="text" value="{{$visitorsleav->id}}" >-->
            <div class="form-group">
                <label class="required" for="visitor_id_num">{{ trans('cruds.visitor.fields.visitor_id_num') }}</label>
                <input class="form-control {{ $errors->has('visitor_id_num') ? 'is-invalid' : '' }}" type="text" name="visitor_id_num" id="visitor_id_num" value="{{ old('visitor_id_num', $visitorsleav->visitor_id_num) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('visitor_name') ? 'is-invalid' : '' }}" type="text" name="visitor_name" id="visitor_name" value="{{ old('visitor_name', $visitorsleav->visitor_name) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('mobile_num') ? 'is-invalid' : '' }}" type="text" name="mobile_num" id="mobile_num" value="{{ old('mobile_num', $visitorsleav->mobile_num) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('office') ? 'is-invalid' : '' }}" name="office_id" id="office_id" required>
                    @foreach($offices as $id => $office)
                        <option value="{{ $id }}" {{ (old('office_id') ? old('office_id') : $visitorsleav->office_id ?? '') == $id ? 'selected' : '' }}>{{ $office }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('governorate') ? 'is-invalid' : '' }}" name="governorate_id" id="governorate_id" required>
                    @foreach($governorates as $id => $governorate)
                        <option value="{{ $id }}" {{ (old('governorate_id') ? old('governorate_id') : $visitorsleav->governorate->id ?? '') == $id ? 'selected' : '' }}>{{ $governorate }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('dept') ? 'is-invalid' : '' }}" name="dept_id" id="dept_id">
                    @foreach($depts as $id => $dept)
                        <option value="{{ $id }}" {{ (old('dept_id') ? old('dept_id') : $visitorsleav->dept->id ?? '') == $id ? 'selected' : '' }}>{{ $dept }}</option>
                    @endforeach
                </select>
                @if($errors->has('dept'))
            ...
            <div class="form-group">
               ...
               <input class="form-control datetime {{ $errors->has('in_date_time') ? 'is-invalid' : '' }}" type="text" name="in_date_time" id="in_date_time" value="{{ old('in_date_time', $visitorsleav->in_date_time) }}"> 
                 @else
                <input readonly class="form-control  {{ $errors->has('in_date_time') ? 'is-invalid' : '' }}" type="text" name="in_date_time" id="in_date_time" value="{{ old('in_date_time', $visitorsleav->in_date_time) }}">
                 @endcan
                {{--
<!--                <input class="form-control datetime {{ $errors->has('in_date_time') ? 'is-invalid' : '' }}" type="text" name="in_date_time" id="in_date_time" value="{{ old('in_date_time', $visitor->in_date_time) }}">-->--}}
            ...
            <div class="form-group">
                ...
                <input  class="form-control datetime  {{ $errors->has('out_date_time') ? 'is-invalid' : '' }}" type="text" name="out_date_time" id="out_date_time" value="{{ old('out_date_time', $visitorsleav->out_date_time) }}">
                 @else
                <input readonly class="form-control   {{ $errors->has('out_date_time') ? 'is-invalid' : '' }}" type="text" name="out_date_time" id="out_date_time" value="<?php echo date("d/m/Y H:i:s"); ?>">
                 @endcan
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('deposit_type') ? 'is-invalid' : '' }}" name="deposit_type_id" id="deposit_type_id">
                 ...
                 <option value="{{ $id }}" {{ (old('deposit_type_id') ? old('deposit_type_id') : $visitorsleav->deposit_type_id ?? '') == $id ? 'selected' : '' }}>{{ $deposit_type }}</option>
           ...
           {{--<option value="{{ $id }}" {{ (old('deposit_type_id') ? old('deposit_type_id') : $visitor->deposit_type->id ?? '') == $id ? 'selected' : '' }}>{{ $deposit_type }}</option>--}}
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('deposit') ? 'is-invalid' : '' }}" name="deposit" id="deposit">{{ old('deposit', $visitorsleav->deposit) }}</textarea>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('deposit_taken') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="deposit_taken" id="deposit_taken" value="1" {{ $visitorsleav->deposit_taken || old('deposit_taken', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('notes') ? 'is-invalid' : '' }}" type="text" name="notes" id="notes" value="{{ old('notes', $visitorsleav->notes) }}">
                @if($errors->has('notes'))
...
<!--
                <select class="form-control select2 {{ $errors->has('acreated_by') ? 'is-invalid' : '' }}" name="acreated_by_id" id="acreated_by_id">
                    @foreach($acreated_bies as $id => $acreated_by)
                        <option value="{{ $id }}" {{ (old('acreated_by_id') ? old('acreated_by_id') : $visitorsleav->acreated_by->id ?? '') == $id ? 'selected' : '' }}>{{ $acreated_by }}</option>
                    @endforeach
                </select>
-->
                      <select class="form-control select2 {{ $errors->has('acreated_by') ? 'is-invalid' : '' }}" name="acreated_by_id" id="acreated_by_id">
                   
                        <option selected readonly value="{{ $visitorsleav->acreated_by_id}}" >{{ $us_name }}</option>
            ...
            <div class="form-group">
                <button onclick="myFunction()"   class="btn btn-danger" type="submit">
                    {{ trans('global.save') }}
                </button>
            </div>
...
<script>
    function myFunction(e) {
         ...
         var deposit_type_id = document.forms["myForm"]["deposit_type_id"].value;

          $(document).ready(function() {
              if(deposit != "" || deposit_type_id != "")
                 {
                     if ($('input.form-check-input').is(':checked')     ) {
               // alert("yyy");
            }else{
                alert("يرجى التأكد من أخذ الايداع");
                     e.preventDefault();
