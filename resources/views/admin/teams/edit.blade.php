...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.teams.update", [$team->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="offices">{{ trans('cruds.team.fields.offices') }}</label>
                ...
                <select class="form-control select2 {{ $errors->has('offices') ? 'is-invalid' : '' }}" name="offices[]" id="offices" multiple>
                    @foreach($offices as $id => $offices)
                        <option value="{{ $id }}" {{ (in_array($id, old('offices', [])) || $team->offices->contains($id)) ? 'selected' : '' }}>{{ $offices }}</option>
                    @endforeach
                </select>
                @if($errors->has('offices'))
                    <span class="text-danger">{{ $errors->first('offices') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.team.fields.offices_helper') }}</span>
            </div>
            <div class="form-group">
